...
def test_initial_snapshot_decision_parity_rejects_when_first_snapshot_hits_limit_on_both_sides() -> None:
    ...
    assert summary.limit_reject_rows == 1
    assert bool(out.loc[0, "decision_quote_limit_reject"])
    assert out.loc[0, "decision_engine_action"] == "reject"
    assert out.loc[0, "decision_engine_reason"] == "orderbook_limit_reject"


def test_initial_snapshot_decision_parity_uses_only_first_snapshot_for_repriced_surface() -> None:
    profile_spec = resolve_backtest_profile_spec(market="sol", profile="deep_otm")
    replay = pd.DataFrame(
        [
            {
