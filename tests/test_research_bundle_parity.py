...
def test_build_model_bundle_can_skip_optional_training_artifacts(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("PM15MIN_MODEL_BUNDLE_COPY_DIAGNOSTICS", "0")
    root = tmp_path / "v2"
    cfg = ResearchConfig.build(
        market="sol",
        cycle="15m",
    ...
    joblib.dump(["ret_1m"], offset_dir / "feature_cols.joblib")
    joblib.dump({"kind": "dummy"}, offset_dir / "models" / "lgbm_sigmoid.joblib")
    (offset_dir / "calibration" / "blend_weights.json").write_text('{"w_lgb": 1.0}', encoding="utf-8")
    (offset_dir / "feature_pruning.json").write_text('{"dropped_columns": ["ma_gap_15"]}', encoding="utf-8")
    (offset_dir / "factor_correlations.parquet").write_text("placeholder", encoding="utf-8")
    (offset_dir / "calibration" / "reliability_bins_blend.json").write_text("[]", encoding="utf-8")
    (offset_dir / "reports" / "offset_report.md").write_text("# Offset Report", encoding="utf-8")
    ...
    (training_run_dir / "summary.json").write_text(
        json.dumps({"offsets": [7], "offset_summaries": [{"offset": 7}]}, indent=2),
        encoding="utf-8",
    )
    ...
    assert (bundle_dir / "offsets" / "offset=7" / "models" / "lgbm_sigmoid.joblib").exists()
    assert not (bundle_dir / "offsets" / "offset=7" / "diagnostics" / "factor_correlations.parquet").exists()
    assert not (bundle_dir / "offsets" / "offset=7" / "reports" / "offset_report.md").exists()
    assert not (bundle_dir / "offsets" / "offset=7" / "calibration" / "reliability_bins_blend.json").exists()
    assert bundle_config["allowed_blacklist_columns"] == ["ma_gap_15"]
    assert payload["optional_offset_artifacts"] == {"offset=7": []}


def test_build_model_bundle_rebuild_removes_stale_offsets(tmp_path) -> None:
    root = tmp_path / "v2"
    cfg = ResearchConfig.build(
        market="sol",
        cycle="15m",
    ...
    training_run_dir = cfg.layout.training_run_dir(model_family="deep_otm", target="direction", run_label_text="source")
    for offset in (7, 8):
        offset_dir = training_run_dir / "offsets" / f"offset={offset}"
        (offset_dir / "models").mkdir(parents=True, exist_ok=True)
        (offset_dir / "calibration").mkdir(parents=True, exist_ok=True)
        (offset_dir / "reports").mkdir(parents=True, exist_ok=True)
        (offset_dir / "feature_schema.json").write_text('[{"name": "ret_1m", "dtype": "float64"}]', encoding="utf-8")
    ...
    (training_run_dir / "summary.json").write_text(
        json.dumps({"offsets": [7, 8], "offset_summaries": []}, indent=2),
        encoding="utf-8",
    )
    ...
    assert (bundle_dir / "offsets" / "offset=8").exists()
    ...
    assert (bundle_dir / "offsets" / "offset=7").exists()
    assert not (bundle_dir / "offsets" / "offset=8").exists()
