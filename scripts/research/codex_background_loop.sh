...
AUTORUN_DIR="$ROOT_DIR/var/research/autorun"
STATUS_PATH="$AUTORUN_DIR/codex-background.status.json"
PID_PATH="$AUTORUN_DIR/codex-background.pid"
LOG_PATH="$AUTORUN_DIR/codex-background.log"
STOP_FLAG="$AUTORUN_DIR/stop.flag"
LAST_PROMPT_PATH="$AUTORUN_DIR/codex-last-prompt.md"
LAST_OUTPUT_PATH="$AUTORUN_DIR/codex-last-output.txt"
FALLBACK_ENV_PATH="${FALLBACK_ENV_PATH:-$AUTORUN_DIR/codex-fallback.env}"
QUEUE_SUPERVISOR_SCRIPT="$ROOT_DIR/scripts/research/experiment_queue_supervisor.sh"

SESSION_DIR="${SESSION_DIR:-}"
PROGRAM_PATH="${PROGRAM_PATH:-$ROOT_DIR/program.md}"
...
run_codex_attempt() {
  ...
  while kill -0 "$attempt_pid" >/dev/null 2>&1; do
    ...
    if [[ -z "$startup_baseline_size" && "$elapsed_sec" -ge 5 ]]; then
      startup_baseline_size="$current_size"
    elif [[ -n "$startup_baseline_size" && "$current_size" -gt "$startup_baseline_size" ]]; then
      startup_progress=1
    fi
...
case "$ACTION" in
  ...
  stop)
    ...
    rm -f "$PID_PATH"
    if [[ -x "$QUEUE_SUPERVISOR_SCRIPT" ]]; then
      "$QUEUE_SUPERVISOR_SCRIPT" stop || true
    fi
    echo "Stop requested"
    ;;
  restart)
