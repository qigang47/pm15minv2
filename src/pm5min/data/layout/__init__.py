...
from pmshared.time import cycle_seconds, normalize_cycle, normalize_surface
from .paths import cycle_asset_file, cycle_asset_root, snapshot_file, snapshot_history_file, year_month_file
...
class MarketDataLayout:
    ...
    def streams_source_root(self) -> Path:
        return self.sources_root / "chainlink" / "streams" / f"asset={self.asset.slug}"

    @property
    def datafeeds_source_root(self) -> Path:
        return self.sources_root / "chainlink" / "datafeeds" / f"asset={self.asset.slug}"
