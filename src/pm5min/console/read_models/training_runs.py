...
def _build_training_run_row(run_dir: Path) -> dict[str, object]:
    ...
    model_family = _string_value(summary, "model_family") or _string_value(spec, "model_family") or _part_value(
        run_dir, "model_family"
    )
...
def _training_explainability_overview(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "offsets_with_factor_direction_summary": sum(
            1 for row in rows if bool(row.get("factor_direction_summary_exists"))
        ),
        ...
        "unique_top_logreg_feature_count": len(
            {str(item.get("feature") or "").strip() for item in top_logreg if str(item.get("feature") or "").strip()}
        ),
        "unique_top_lgb_feature_count": len(
            {str(item.get("feature") or "").strip() for item in top_lgb if str(item.get("feature") or "").strip()}
        ),
...
def _bundle_readiness_entry(row: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "calibration_file_count": len(
            row.get("calibration_files") if isinstance(row.get("calibration_files"), list) else []
        ),
    }
