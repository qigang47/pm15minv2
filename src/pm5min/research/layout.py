...
class ResearchLayout:
    ...
    def experiment_run_dir(self, suite_name: str, run_label_text: str) -> Path:
        return self.experiment_runs_root / f"suite={slug_token(suite_name)}" / f"run={slug_token(run_label_text, default='planned')}"
...
class MarketResearchLayout:
    ...
    def feature_frame_dir(self, feature_set: str, *, source_surface: str = "backtest") -> Path:
        return self.feature_frames_root / f"feature_set={slug_token(feature_set)}" / f"source_surface={normalize_source_surface(source_surface)}"
    ...
    def training_set_dir(self, *, feature_set: str, label_set: str, target: str, window: str, offset: int) -> Path:
        return (
            self.training_sets_root
    ...
    def training_set_path(self, *, feature_set: str, label_set: str, target: str, window: str, offset: int) -> Path:
        return self.training_set_dir(feature_set=feature_set, label_set=label_set, target=target, window=window, offset=offset) / "data.parquet"
