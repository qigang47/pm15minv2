...
from pm5min.research.bundles.active_registry import resolve_active_bundle_dir
...
def resolve_training_run_dir(
    ...
    *,
    ...
    root = cfg.layout.training_runs_root / f"model_family={slug_token(cfg.model_family)}" / f"target={normalize_target(target)}"
    if source_training_run:
        direct = root / f"run={slug_token(source_training_run)}"
        if direct.exists():
            return direct
...
def resolve_model_bundle_dir(
    ...
    *,
    ...
    root = cfg.layout.model_bundles_root / f"profile={slug_token(profile)}" / f"target={normalize_target(target)}"
    if bundle_label:
        direct = root / f"bundle={slug_token(bundle_label)}"
        if direct.exists():
            return direct
