...
from .layout import MarketResearchLayout, ResearchLayout
from .layout_helpers import normalize_cycle, normalize_source_surface, normalize_target, slug_token


def normalize_label_set(label_set: str) -> str:
    token = str(label_set or "truth").strip().lower()
    if token in {"truth", "settlement_truth"}:
        return "truth"
    return slug_token(token)
