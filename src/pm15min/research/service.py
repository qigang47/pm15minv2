...
from pm15min.core.config import ResearchConfig
from pm15min.research.bundles.active_registry import read_active_bundle_selection, write_active_bundle_selection
from pm15min.research.bundles.loader import resolve_model_bundle_dir, read_model_bundle_manifest


def describe_research_runtime(cfg: ResearchConfig) -> dict[str, object]:
    return {
        "domain": "research",
...
def list_training_runs(
    ...
    *,
    ...
    pattern = "model_family=*/target=*/run=*"
    runs = [path for path in root.glob(pattern) if path.is_dir()]
    if model_family:
        token = f"model_family={model_family}"
        runs = [path for path in runs if path.parent.parent.name == token]
    if target:
        token = f"target={target}"
        runs = [path for path in runs if path.parent.name == token]
    if prefix:
        token = f"run={prefix}"
        runs = [path for path in runs if path.name.startswith(token)]
...
def list_model_bundles(
    ...
    *,
    ...
    if profile:
        token = f"profile={profile}"
        bundles = [path for path in bundles if path.parent.parent.name == token]
    if target:
        token = f"target={target}"
        bundles = [path for path in bundles if path.parent.name == token]
    if prefix:
        token = f"bundle={prefix}"
        bundles = [path for path in bundles if path.name.startswith(token)]
...
def list_artifact_runs(market: str, prefix: str | None = None) -> list[dict[str, str]]:
    cfg = ResearchConfig.build(market=market, profile="default")
    return list_training_runs(cfg, prefix=prefix)
