...
`reversal` line in `sol+xrp` mode.

## Canonical References

- Active session:
  `sessions/deep_otm_baseline_reversal_dense_sol_xrp_autoresearch/session.md`
- Active results log:
  `sessions/deep_otm_baseline_reversal_dense_sol_xrp_autoresearch/results.tsv`
...
- SOL and XRP must be routed independently:
  - XRP reversal around the current `30-40` trade band should prioritize density
    release around the best capture family until it reaches `56+`
  - SOL reversal below `20` trades should skip weight-only cosmetic retries and
...
- this reversal track owns up to `5` quick-screen slots; do not stop at one
