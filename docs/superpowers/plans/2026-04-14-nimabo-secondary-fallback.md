...
**Goal:** Add a background-only second Nimabo API key fallback and remove the deprecated third-party fallback, without changing the operator's normal manual Codex setup.
...
- [ ] Step 2: Add tests that require the README to document the new order: primary Nimabo, secondary Nimabo key, optional configured backup provider, then official login fallback.
...
- [ ] Step 4: Keep the later generic backup-provider and official-login retries available, but do not document or configure the deprecated provider.
